<?php
// Lightweight DB connectivity checker. Does not include full app bootstrap.
...
<head>
  ...
  <style>body{font-family:system-ui,Segoe UI,Helvetica,Arial;margin:20px;} code{background:#f3f3f3;padding:2px 6px;border-radius:4px;}</style>
</head>
<body>
  <h1>Database status</h1>
  <p>Host: <strong><?= htmlspecialchars($db_host) ?></strong> Port: <strong><?= htmlspecialchars($db_port) ?></strong> DB: <strong><?= htmlspecialchars($db_name) ?></strong></p>
  <?php if ($ok): ?>
